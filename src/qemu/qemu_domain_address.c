                                         ...
                                         virDomainPCIConnectFlags virtioFlags)
{
    ...
    case VIR_DOMAIN_DEVICE_IOMMU:
        switch (dev->data.iommu->model) {
            ...
            case VIR_DOMAIN_IOMMU_MODEL_SMMUV3_ACCEL:
                              ...
                              bool dryRun)
{
    ...
    if (addrs->dryRun && def->iommu &&
        ...
        for (i = 0; i < def->niommus; i++) {
            if (!virDeviceInfoPCIAddressIsWanted(def->iommu[i]->info))
                continue;
...
static char *
smmuv3AccelVfioHostdevFound(virDomainHostdevDef *hostdev, bool dryRun)
{
    char* devPath = NULL;
...
static int
qemuDomainAssignSmmuv3AccelHostdevSlots(virDomainDef *def,
                                         virDomainPCIAddressSet *addrs)
{
    size_t i, j;
    char* smmu_node = NULL;
    ...
    if (def->iommu && def->iommu->model == VIR_DOMAIN_IOMMU_MODEL_SMMUV3_ACCEL) {
        for (i = 0; i < def->nhostdevs; i++) {
            if (!(smmu_node = smmuv3AccelVfioHostdevFound(def->hostdevs[i], addrs->dryRun)))
                continue;
            /* Find a hostdev and nested SMMU pair */
            for (j = 0; j < def->niommus; j++) {
                unsigned int smmuv3AccelBus = def->iommu[j]->info->addr.pci.bus;
                virDomainControllerDef *pxb;
                if (!STREQLEN(def->iommu[j]->host_smmu, smmu_node, strlen(smmu_node)))
                    continue;
                ...
                set = virDomainPCIAddressSetAlloc(smmuv3AccelBus + 1,
                                                  VIR_PCI_ADDRESS_EXTENSION_NONE);
                set->buses[smmuv3AccelBus] = addrs->buses[smmuv3AccelBus];
                               ...
                               virDomainPCIAddressSet *addrs)
{
    ...
    /* smmuv3Accel devices */
    if (!addrs->dryRun && def->iommu->model == VIR_DOMAIN_IOMMU_MODEL_SMMUV3_ACCEL) {
        for (i = 0; i < def->niommus; i++) {
            if (!virDeviceInfoPCIAddressIsWanted(def->iommu[i]->info))
                continue;
    ...
    if (!addrs->dryRun && def->iommu->model == VIR_DOMAIN_IOMMU_MODEL_SMMUV3_ACCEL) {
        if (qemuDomainAssignSmmuv3AccelHostdevSlots(def, addrs) < 0)
            return -1;
    }
