...
qemuProcessOpenVfioFds(virDomainObj *vm)
{
    ...
    for (i = 0; i < vm->def->nhostdevs; i++) {
        ...
        if (hostdev->mode == VIR_DOMAIN_HOSTDEV_MODE_SUBSYS &&
            ...
            if (hostdev->source.subsys.u.pci.driver.name == VIR_DEVICE_HOSTDEV_PCI_DRIVER_NAME_VFIO &&
                ...
                fdpass = qemuFDPassNew(fdname, &vfioFd);
                qemuFDPassAddFD(fdpass, &vfioFd, NULL);
                g_hash_table_insert(priv->fds, g_steal_pointer(&fdname), fdpass);
    ...
    if (needsIommuFd) {
        ...
        fdpass = qemuFDPassNew("iommufd0", priv);
        qemuFDPassAddFD(fdpass, &iommuFd, NULL);
        g_hash_table_insert(priv->fds, g_strdup("iommufd0"), fdpass);
...
qemuProcessCloseVfioFds(virDomainObj *vm)
{
    ...
    GHashTableIter iter;
    gpointer key, value;
    g_autoptr(GPtrArray) keysToRemove = g_ptr_array_new_with_free_func(g_free);
    size_t i;

    /* Find and remove VFIO/IOMMU FDs from the general fds table */
    g_hash_table_iter_init(&iter, priv->fds);
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        const char *fdname = key;

        if (STRPREFIX(fdname, "vfio-") || STREQ(fdname, "iommufd0")) {
            g_ptr_array_add(keysToRemove, g_strdup(fdname));
        }
    }

    /* Remove the keys (this will auto-cleanup the FDs) */
    for (i = 0; i < keysToRemove->len; i++) {
        g_hash_table_remove(priv->fds, g_ptr_array_index(keysToRemove, i));
    }
}
