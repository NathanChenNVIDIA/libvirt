...
qemuProcessOpenVfioFds(virDomainObj *vm)
{
    ...
    for (i = 0; i < vm->def->nhostdevs; i++) {
        ...
        if (hostdev->mode == VIR_DOMAIN_HOSTDEV_MODE_SUBSYS &&
            ...
            if (hostdev->source.subsys.u.pci.driver.name == VIR_DEVICE_HOSTDEV_PCI_DRIVER_NAME_VFIO &&
                ...
                fdpass = qemuFDPassNew(fdname, priv);
                qemuFDPassAddFD(fdpass, &vfioFd, NULL);
                g_hash_table_insert(priv->fdpasses, g_steal_pointer(&fdname), fdpass);
    ...
    if (needsIommuFd) {
        ...
        g_hash_table_insert(priv->fdpasses, g_strdup("iommufd0"), fdpass);
...
qemuProcessCloseVfioFds(virDomainObj *vm)
{
    qemuDomainObjPrivate *priv = vm->privateData;
    if (priv->fdpasses)
        g_hash_table_remove_all(priv->fdpasses);
}
