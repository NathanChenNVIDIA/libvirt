                                   ...
                                   bool fromConfig)
{
    ...
    if (!(devFlags & busFlags & VIR_PCI_CONNECT_TYPES_MASK)) {
        ...
        } else if (devFlags & VIR_PCI_CONNECT_TYPE_SMMUV3_ACCEL) {
            connectStr = "smmuv3Accel";
        } else {
                               ...
                               bool allowHotplug)
{
    ...
    case VIR_DOMAIN_CONTROLLER_MODEL_PCIE_EXPANDER_BUS:
        ...
        bus->flags = (VIR_PCI_CONNECT_TYPE_PCIE_ROOT_PORT |
                      VIR_PCI_CONNECT_TYPE_DMI_TO_PCI_BRIDGE |
                      VIR_PCI_CONNECT_TYPE_SMMUV3_ACCEL);
        bus->minSlot = 0;
        bus->maxSlot = VIR_PCI_ADDRESS_SLOT_LAST;
        break;
                           ...
                           virDomainPCIConnectFlags flags)
{
    ...
    } else if (flags & VIR_PCI_CONNECT_TYPE_SMMUV3_ACCEL) {
    ...
    *found = false;
    ...
    if (!virDomainPCIAddressFlagsCompatible(searchAddr, addrStr, bus->flags,
        ...
        while (searchAddr->slot <= bus->maxSlot) {
            ...
            if (flags == VIR_PCI_CONNECT_TYPE_SMMUV3_ACCEL) {
                *found = false;
                break;
            }
